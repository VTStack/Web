...
function readDefaultTsConfig(tsConfigPath) {
    ...
    if (tsConfigPath && (0, fs_1.existsSync)(tsConfigPath)) {
        try {
            ...
            const { options, errors, fileNames } = ts.parseJsonConfigFileContent(config, ts.sys, (0, path_1.dirname)(tsConfigPath));
            if (!errors.length) {
                compilerOptions = options;
                compilerOptions.files = fileNames;
            }
            else {
                console.info(chalk_1.default.yellow(`Convert compiler options from json failed, ${errors.map((d) => d.messageText).join('\n')}`));
            }
        }
        catch (e) {
            console.info(chalk_1.default.yellow(`Read ${tsConfigPath} failed: ${e.message}`));
        }
    }
